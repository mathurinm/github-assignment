...
class OneNearestNeighbor(BaseEstimator, ClassifierMixin):
    ...
    def predict(self, X):
        ...
        for i, x in enumerate(X):
            nearest_idx = np.argmin(np.linalg.norm(self._X_train - x, axis=1))
    ...
    def score(self, X, y):
        ...
        -----
        X : numpy.ndarray
            The input data for which to calculate the accuracy.the X test
            dataset

        y : numpy .ndarray
            The true class labels corresponding to the input data.y_test
            array
